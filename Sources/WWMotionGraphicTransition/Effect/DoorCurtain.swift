...
public extension WWMotionGraphicTransition.DoorCurtain {
    ...
    static func maker(frame: CGRect) -> WWMotionGraphicTransition.DoorCurtain {
...
public extension WWMotionGraphicTransition.DoorCurtain {
    ...
    ///   - count: 門簾數量
    ///   - colors: 門簾顏色
    func start(duration: TimeInterval = 0.5, count: Int = 3, colors: [UIColor] = [.red, .yellow, .green]) {
        ...
        (0..<count).forEach { index in
            ...
            demoView.frame.origin.x = -frame.width
            
            let animator = UIViewPropertyAnimator(duration: duration, curve: .easeIn) {
                demoView.frame.origin.x = 0
            }
    ...
    func end(duration: TimeInterval = 0.5) {
        ...
        for (index, demoView) in self.subviews.reversed().enumerated() {
            ...
            let animator = UIViewPropertyAnimator(duration: duration, curve: .easeIn) {
                demoView.frame.origin.x = self.frame.width
            }
